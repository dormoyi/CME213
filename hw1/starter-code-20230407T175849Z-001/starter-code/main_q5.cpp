...
// std::for_each applies f inplace
// std::transform applies f and stores in another container
...
std::vector<T> daxpy(T a, const std::vector<T>& x, const std::vector<T>& y) {
  ...
  // transform(Iterator inputBegin1, Iterator inputEnd1, 
        //  Iterator inputBegin2, Iterator OutputBegin, operation) 
  return res;
  //return std::vector<T>();
}
...
bool all_students_passed(const std::vector<Student>& students,
  ...
  for (auto student : students) {
    ...
    student.midterm*MIDTERM_WEIGHT + student.final_exam*FINAL_EXAM_WEIGHT;
...
bool wayToSort(int i, int j) { // if false, j is first
  if (i%2 != 0 && j%2 == 0) 
    return true;
  else if (i%2 == 0 && j%2 != 0) 
    return false;
