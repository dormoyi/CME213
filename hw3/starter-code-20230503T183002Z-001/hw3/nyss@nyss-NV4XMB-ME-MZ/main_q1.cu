...
class RecurrenceTestFixture : public ::testing::Test {
 protected:
  ...
  void save(std::vector<double> &performance_array, std::string name){
    std::ofstream myfile;
    myfile.open (name);
    for (auto v:performance_array)
        myfile << v << " ";
    myfile.close();
  }
};
...
TEST_F(RecurrenceTestFixture, RecurrenceThreadsTest_4) {
  ...
  cout << endl;
  save(performance_array, "Q1.4.csv");
}
...
TEST_F(RecurrenceTestFixture, RecurrenceBlocksTest_5) {
  ...
  cout << endl;
  save(performance_array, "Q1.5.csv");
}
...
TEST_F(RecurrenceTestFixture, RecurrenceItersTest_6) {
  ...
  cout << endl;
  save(performance_array, "Q1.6.csv");
}
